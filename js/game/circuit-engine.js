...
const CircuitEngine = (function() {
  ...
  function evaluate(levelConfig) {
    ...
    if (goal.type === 'series') {
      return evaluateSeriesGoal(battery, bulbs, requiredBulbs, switches, requiredSwitches, components);
    }
  ...
  function evaluateSeriesGoal(battery, bulbs, requiredBulbs, switches, requiredSwitches, components) {
    const startKey = battery.uid + '.pos';
    const targetKey = battery.uid + '.neg';
    const visited = new Set();
    const queue = [{ key: startKey, path: [startKey] }];
    ...
    while (queue.length > 0) {
      ...
      if (current.key === targetKey) {
        ...
        // Series check: ALL bulbs must be on the SAME single path
        if (bulbsInPath.length !== bulbs.length) {
          return { status: 'open', reason: 'not-series', message: 'Circuit error: All bulbs must be on the same path (series circuit).' };
        }
        ...
        if (switchesInPath.length < requiredSwitches) {
          return { status: 'open', reason: 'missing-switch', message: 'Missing switch in circuit.' };
        }

        return {
          status: 'closed',
          message: 'Series circuit complete!',
          path: current.path,
          battery: battery,
          bulb: bulbsInPath[0],
          bulbs: bulbsInPath
